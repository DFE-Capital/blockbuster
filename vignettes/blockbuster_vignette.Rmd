...
The output of the `blockbuster` function is a list of dataframes. The number of dataframes is equivalent to the number of years simulated plus one. The output contains all the variables from the input initial `blockbuster_tibble`. Here we simulate the deterioration of ten building components through one year. We are interested in the variables of a simulated PDS produced by our `blockbuster` model using default arguments.    
