---
title: "Blockbuster RAM usage"
author: "Matthew Gregory"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{blockbuster_memory_usage}
...
We know the memory change and object size from earlier in this document. How does the maximum Memory used at any time compare to this value?  

In this simple example most of the memory and time is spent in the `blockbust` function which is concerned with the deterioration of each building component.  

### With Rebuild an repair

It may be the case that the rebuild and repair consume a lot of memory. Careful putting sensible values for the rebuild monies, it can error if its insufficient to rebuild the cheapest block.     
...
y <- blockbuster(x, forecast_horizon = 5, rebuild_monies = c(0, 0, 50e4, 0, 0),
                 repair_monies = rep(1e4, 5))
  
})
```

The repairing and rebuilding reduces the number of rows in the dataframe for that timestep thus reducing the memory issue but also affects how long it takes the code to run. In this example it is reducing the number of rows that need to be deteriorated (as for each building component the worse condition rows will be repaired thus making more of the `unit_area` of the same `grade`, requiring fewer calls to `det_teriorate`).    

## Potential work-arounds

It looks like the deterioration of the dataframes at each timestep are held in memory thus their growth and the forecast horizon are the biggest impacters on RAM being used with both scaling linearly. The solution will be to reduce what is held in RAM. The simulation only needs to hold the current timestep and the next one (as it's deteriorating and rebuilding and repairing in RAM), the historic timesteps are no longer referenced too in that simulation, thus could rest be written to disk.  

However, we also need to consider how to scale this to the eventual 100 % of the PDS. We are currently using only 10% sub-sample. Writing to disk once the current timestep's data frame is needed would drastically reduce the demand on RAM. An alternative would be to rewriting the package using data.table. Writing to disk frequnetly is not ideal and may slow things down but the important thing is that the simulations can be run at all.    
